...
// Dynamic Announcement Ticker State (Konsisten dengan Home_Navbar)
const announcements = ref([
  '✝ Soli Deo Gloria — Pelayanan Jemaat & Sistem Informasi Gereja',
  '⛪ Cari Jadwal Ibadah, Lokasi & Pelayanan Gereja Terintegrasi',
  '📞 Kontak Sekretariat & Layanan Doa: (021) 555-0199 / WhatsApp 0812-3456-7890',
  '📖 Sistem Integrasi CMS Gereja — Terhubung di Seluruh Indonesia',
  '🕊️ Perjamuan Kudus & Ibadah Raya Terbuka Untuk Seluruh Jemaat',
])
...
<template>
  <header class="classic-navbar sticky top-0 z-50 bg-[#0f172a] text-slate-100 shadow-2xl border-b border-amber-500/30">
    <!-- Top Announcement Bar (Konsisten dengan Home_Navbar) -->
    <div class="bg-slate-800 border-b border-amber-500/40 text-amber-100 py-1 px-3 sm:px-4 text-center font-serif text-[10px] sm:text-xs tracking-wider uppercase flex justify-center items-center h-7 sm:h-8 relative overflow-hidden shadow-inner">
      <Transition name="fade-slide" mode="out-in">
        <div 
          ...
          class="flex items-center justify-center gap-1.5 sm:gap-2 w-full text-amber-200 font-semibold drop-shadow-sm px-1 sm:px-2"
        >
      ...
      <div class="flex items-center justify-between h-16 sm:h-20">
        ...
        <div class="flex items-center gap-1.5 sm:gap-3 min-w-0">
          <button 
            ...
            type="button"
            class="p-1.5 text-amber-300 hover:text-amber-100 hover:bg-slate-900 border border-amber-500/30 rounded transition flex items-center justify-center shrink-0 cursor-pointer sm:p-1.5"
          >
          ...
          <RouterLink to="/" class="flex min-w-0 max-w-[60vw] items-center gap-1.5 sm:gap-2 group cursor-pointer no-underline sm:max-w-none">
            <div class="h-8 w-8 shrink-0 overflow-hidden rounded-full border-2 border-slate-400/70 bg-gradient-to-b from-slate-900 to-slate-950 shadow-md shadow-slate-500/10 transition-all duration-300 group-hover:border-slate-200 sm:h-9 sm:w-9">
              <img src="@/assets/images/GracePoint.png" alt="GracePoint Logo" class="h-full w-full object-cover">
            </div>

            <div class="flex min-w-0 flex-col">
              <div class="flex items-center gap-1.5">
                <span class="block truncate font-serif text-[0.72rem] font-bold tracking-[0.08em] text-amber-300 transition group-hover:text-amber-200 sm:text-[0.85rem] lg:text-base lg:tracking-[0.12em]">
                  GRACEPOINT
                </span>
              </div>
              <span class="block truncate text-[6.5px] font-serif uppercase tracking-[0.12em] text-slate-200 opacity-95 sm:text-[7.5px] lg:text-[8.5px] lg:tracking-[0.18em]">
        ...
        <nav class="hidden md:flex items-center gap-3 lg:gap-6" aria-label="Navigasi utama">
          <a 
            ...
            :class="[
              'classic-link font-sans text-[11px] uppercase font-medium tracking-wider text-white hover:text-amber-300 py-1 transition-colors duration-200 relative',
          ...
          <a 
            href="/informasi-pelayanan#info" 
            @click.prevent="navigateToAnchor('/informasi-pelayanan', '#info')"
            :class="[
              'classic-link font-sans text-[11px] uppercase font-medium tracking-wider text-white hover:text-amber-300 py-1 transition-colors duration-200 relative',
              route.path === '/informasi-pelayanan' && route.hash !== '#details' ? 'text-amber-400 font-semibold active-link' : ''
            ]"
          >
            Informasi Pelayanan
          </a>
          ...
          <a 
            href="/informasi-pelayanan#details" 
            @click.prevent="navigateToAnchor('/informasi-pelayanan', '#details')"
            :class="[
              'classic-link font-sans text-[11px] uppercase font-medium tracking-wider text-white hover:text-amber-300 py-1 transition-colors duration-200 relative',
              route.hash === '#details' ? 'text-amber-400 font-semibold active-link' : ''
            ]"
          >
            Lihat Peta (Map)
          </a>
        ...
        <div class="flex items-center gap-1.5 sm:gap-3 flex-shrink-0">
          ...
          <div v-if="isAuthenticated && user?.role !== 'superadmin'" class="flex min-w-0 items-center gap-1.5 border-l border-amber-500/20 pl-2 sm:pl-2.5">
            <div class="flex max-w-[90px] min-w-0 flex-col text-right xs:max-w-[120px] sm:max-w-[180px]">
              <span class="truncate font-serif text-[9px] font-bold tracking-wide text-amber-300 sm:text-[10px]">{{ user?.name || 'Anggota Jemaat' }}</span>
              <span class="truncate text-[6.5px] font-serif uppercase tracking-[0.16em] text-slate-300 sm:text-[7px]">{{ user?.role === 'church_admin' ? 'Admin Gereja' : 'Jemaat' }}</span>
            </div>
            ...
            <button 
              ...
              title="Keluar"
              type="button"
              class="p-1.25 text-amber-300/80 hover:text-amber-300 hover:bg-amber-950/60 rounded border border-amber-500/30 transition cursor-pointer shrink-0"
            >
              <svg class="nav-icon w-3 h-3 sm:w-3.5 sm:h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <button 
            ...
            type="button"
            class="inline-flex shrink-0 items-center justify-center gap-1 rounded border border-amber-500/60 bg-amber-500/10 px-2 py-1.25 text-[8.5px] font-serif uppercase tracking-[0.08em] text-amber-300 shadow-sm transition duration-300 hover:bg-amber-400 hover:text-slate-950 sm:px-2.5 sm:py-1.5 sm:text-[10px]"
          >
            <svg class="h-3 w-3 shrink-0 sm:h-3.5 sm:w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 16l-4-4m0 0l4-4m-4 4h14m-5 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/>
            </svg>
            <span class="hidden sm:inline">Masuk / Daftar</span>
            <span class="sm:hidden text-[8px]">Akses</span>
          </button>
        ...
        >
          ...
          <div class="flex items-center justify-between pb-3 border-b border-amber-500/20 mb-3">
            <div class="flex items-center gap-2.5">
              ...
              <div class="flex flex-col">
                ...
                <span class="text-[9px] font-serif text-slate-400 uppercase tracking-wider">Informasi Pelayanan Gereja</span>
              </div>
            </div>
          ...
          <div class="space-y-1.5">
            ...
            <a 
              ...
              @click.prevent="navigateToAnchor('/informasi-pelayanan', '#info')"
              class="flex items-center justify-between px-3.5 py-2 rounded-xl border border-transparent hover:border-amber-500/30 hover:bg-amber-500/10 text-slate-200 hover:text-amber-300 transition text-xs font-medium"
              :class="route.path === '/informasi-pelayanan' && route.hash !== '#details' ? 'bg-amber-500/15 border-amber-500/30 text-amber-300 font-bold' : ''"
            >
              <span class="flex items-center gap-2.5">
                <span>⛪</span>
                <span>Informasi Pelayanan</span>
              </span>
              <span class="text-[10px] text-amber-400 font-mono">Info</span>
            </a>
            ...
            <a 
              ...
              @click.prevent="navigateToAnchor('/informasi-pelayanan', '#details')"
              class="flex items-center justify-between px-3.5 py-2 rounded-xl border border-transparent hover:border-amber-500/30 hover:bg-amber-500/10 text-slate-200 hover:text-amber-300 transition text-xs font-medium"
              :class="route.hash === '#details' ? 'bg-amber-500/15 border-amber-500/30 text-amber-300 font-bold' : ''"
            >
              <span class="flex items-center gap-2.5">
                <span>🗺️</span>
                <span>Peta Lokasi & Geolocation</span>
              </span>
              <span class="text-[10px] text-amber-400 font-mono">Map</span>
            </a>
            ...
            <RouterLink 
              ...
              @click="isMobileMenuOpen = false"
              class="flex items-center justify-between px-3.5 py-2 rounded-xl border border-transparent hover:border-amber-500/30 hover:bg-amber-500/10 text-slate-200 hover:text-amber-300 transition text-xs font-medium"
              :class="route.path === '/hari-besar' ? 'bg-amber-500/15 border-amber-500/30 text-amber-300 font-bold' : ''"
            >
              <span class="flex items-center gap-2.5">
                <span>🗓️</span>
                <span>Hari Besar & Kalender</span>
              </span>
              <span class="text-[10px] text-amber-400 font-mono">Events</span>
            </RouterLink>
            ...
            <RouterLink 
              ...
              @click="isMobileMenuOpen = false"
              class="flex items-center justify-between px-3.5 py-2 rounded-xl border border-transparent hover:border-amber-500/30 hover:bg-amber-500/10 text-slate-200 hover:text-amber-300 transition text-xs font-medium"
              :class="route.path === '/berita-jemaat' ? 'bg-amber-500/15 border-amber-500/30 text-amber-300 font-bold' : ''"
            >
              <span class="flex items-center gap-2.5">
                <span>📰</span>
                <span>Warta & Berita Jemaat</span>
              </span>
              <span class="text-[10px] text-amber-400 font-mono">News</span>
            </RouterLink>
            ...
            >
              <span class="flex items-center gap-2.5">
                <span>📜</span>
                <span>Sejarah Gereja Nusantara</span>
              </span>
